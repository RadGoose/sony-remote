...
bool CameraDevice::connect(SCRSDK::CrSdkControlMode openMode, SCRSDK::CrReconnectingSet reconnect)
{
    ...
    // Read credentials from environment (set by GUI)
    const char* envUser = std::getenv("SONY_REMOTE_USER");
    const char* envPass = std::getenv("SONY_REMOTE_PASS");
    const char* userId = (envUser && envUser[0]) ? envUser : "admin";
    const char* userPass = (envPass) ? envPass : "";

    // Auto-get SSH fingerprint if needed
    if (SDK::CrSSHsupportValue::CrSSHsupport_ON == get_sshsupport())
    {
        ...
        m_userPassword = std::string(userPass);
    }

    tout << "Connecting as '" << userId << "'...\n";

    m_spontaneous_disconnection = false;
    auto connect_status = SDK::Connect(m_info, this, &m_device_handle, openMode, reconnect, userId, m_userPassword.c_str(), m_fingerprint.c_str(), (CrInt32u)m_fingerprint.size());
...
void CameraDevice::live_view_stream(int target_fps)
{
    // Enable live view
    ...
    auto frame_path = fs::current_path();
    frame_path.append(TEXT("LiveView000000.JPG"));

    // Wait for live view to become available, retrying up to 5 seconds
    CrInt32u bufSize = 0;
    ...
    auto* image_buff = new CrInt8u[bufSize];
    image_data->SetSize(bufSize);
    image_data->SetData(image_buff);

    int min_frame_ms = (target_fps > 0) ? (1000 / target_fps) : 0;
    int frame_count = 0;

    // Read stdin non-blocking to check for "stop" command
    // We'll just loop until stdin has data
    while (true)
    {
        auto t_start = std::chrono::steady_clock::now();

        auto err = SDK::GetLiveViewImage(m_device_handle, image_data);
        if (CR_SUCCEEDED(err) && image_data->GetImageSize() > 0)
        {
            std::ofstream file(frame_path, std::ios::out | std::ios::binary | std::ios::trunc);
            if (!file.bad())
            {
                file.write((char*)image_data->GetImageData(), image_data->GetImageSize());
                file.close();
            ...
            // Query AF, tracking, and face frames for overlay
            CrInt32u codes[3] = {
                SDK::CrLiveViewPropertyCode::CrLiveViewProperty_AF_Area_Position,
                SDK::CrLiveViewPropertyCode::CrLiveViewProperty_TrackingFrame,
                SDK::CrLiveViewPropertyCode::CrLiveViewProperty_FaceFrame
            };
            CrInt32 propNum = 0;
            SDK::CrLiveViewProperty* props = nullptr;
            auto propErr = SDK::GetSelectLiveViewProperties(m_device_handle, 3, codes, &props, &propNum);

            // Format: LVSTREAM_FRAME N|AF:...|TRK:...|FACE:...
            ...
            tout << "LVSTREAM_FRAME " << frame_count;

            if (CR_SUCCEEDED(propErr) && props) {
                for (int p = 0; p < propNum; p++) {
                    auto& prop = props[p];
                    if (SDK::CrFrameInfoType::CrFrameInfoType_FocusFrameInfo == prop.GetFrameInfoType()) {
                        int cnt = prop.GetValueSize() / sizeof(SDK::CrFocusFrameInfo);
                        auto* fi = (SDK::CrFocusFrameInfo*)prop.GetValue();
                        tout << "|AF:";
                        for (int i = 0; fi && i < cnt; i++) {
                            if (i > 0) tout << ";";
                            tout << fi[i].xNumerator << "," << fi[i].yNumerator << ","
                                 << fi[i].xDenominator << "," << fi[i].yDenominator << ","
                                 << fi[i].width << "," << fi[i].height << ","
                                 << (int)fi[i].state;
                        }
                    }
                    else if (SDK::CrFrameInfoType::CrFrameInfoType_TrackingFrameInfo == prop.GetFrameInfoType()) {
                        int cnt = prop.GetValueSize() / sizeof(SDK::CrTrackingFrameInfo);
                        auto* ti = (SDK::CrTrackingFrameInfo*)prop.GetValue();
                        tout << "|TRK:";
                        for (int i = 0; ti && i < cnt; i++) {
                            if (i > 0) tout << ";";
                            tout << ti[i].xNumerator << "," << ti[i].yNumerator << ","
                                 << ti[i].xDenominator << "," << ti[i].yDenominator << ","
                                 << ti[i].width << "," << ti[i].height << ","
                                 << (int)ti[i].type << "," << (int)ti[i].state;
                        }
                    }
                    else if (SDK::CrFrameInfoType::CrFrameInfoType_FaceFrameInfo == prop.GetFrameInfoType()) {
                        int cnt = prop.GetValueSize() / sizeof(SDK::CrFaceFrameInfo);
                        auto* ffi = (SDK::CrFaceFrameInfo*)prop.GetValue();
                        tout << "|FACE:";
                        for (int i = 0; ffi && i < cnt; i++) {
                            if (i > 0) tout << ";";
                            tout << ffi[i].xNumerator << "," << ffi[i].yNumerator << ","
                                 << ffi[i].xDenominator << "," << ffi[i].yDenominator << ","
                                 << ffi[i].width << "," << ffi[i].height << ","
                                 << (int)ffi[i].state;
                        }
                    }
                }
                SDK::ReleaseLiveViewProperties(m_device_handle, props);
            }
            tout << "\n";
            std::flush(tout);
        }
        ...
        {
            // Camera hasn't produced a new frame yet, just retry quickly
            std::this_thread::sleep_for(5ms);
            continue;
        }
        else
        {
            // Real error - try to continue
        ...
        {
            std::this_thread::sleep_for(std::chrono::milliseconds(min_frame_ms - elapsed_ms));
        }

        // Check if stdin has a line ready (non-blocking check)
        // We use select() on stdin to see if there's input
        fd_set fds;
        ...
        if (select(STDIN_FILENO + 1, &fds, nullptr, nullptr, &tv) > 0)
        {
            ...
            // Handle "af X Y" command during streaming
            if (line.substr(0, 3) == TEXT("af "))
            {
                // Release any previous S1 hold before setting new position
                SDK::CrDeviceProperty s1rel;
                s1rel.SetCode(SDK::CrDevicePropertyCode::CrDeviceProperty_S1);
                s1rel.SetCurrentValue(SDK::CrLockIndicator::CrLockIndicator_Unlocked);
                s1rel.SetValueType(SDK::CrDataType::CrDataType_UInt16);
                SDK::SetDeviceProperty(m_device_handle, &s1rel);
                std::this_thread::sleep_for(50ms);

                text_stringstream ss(line.substr(3));
                int ax = 0, ay = 0;
                ss >> ax >> ay;
                set_af_area_position_xy(ax, ay);
            }
            // Handle "afrelease" - release S1 half-press
            else if (line == TEXT("afrelease"))
            {
                ...
                SDK::SetDeviceProperty(m_device_handle, &s1);
                tout << "AF_RELEASED\n";
                std::flush(tout);
            }
            // Handle "afmode N" - set focus mode
            else if (line.substr(0, 7) == TEXT("afmode "))
            {
                text_stringstream ss(line.substr(7));
                int mode = 0;
                ss >> mode;
                SDK::CrDeviceProperty prop;
                prop.SetCode(SDK::CrDevicePropertyCode::CrDeviceProperty_FocusMode);
                prop.SetCurrentValue((CrInt64u)mode);
                prop.SetValueType(SDK::CrDataType::CrDataType_UInt16Array);
                SDK::SetDeviceProperty(m_device_handle, &prop);
                tout << "AFMODE_SET " << mode << "\n";
                std::flush(tout);
            }
            // Handle "afarea N" - set focus area
            else if (line.substr(0, 7) == TEXT("afarea "))
            {
                text_stringstream ss(line.substr(7));
                int area = 0;
                ss >> area;
                ...
                prop.SetCurrentValue((CrInt64u)area);
                prop.SetValueType(SDK::CrDataType::CrDataType_UInt16Array);
                SDK::SetDeviceProperty(m_device_handle, &prop);
                tout << "AFAREA_SET " << area << "\n";
                std::flush(tout);
            }
            // Handle "shoot" command during streaming
            else if (line == TEXT("shoot"))
            {
                capture_image();
                tout << "CAPTURE_DONE\n";
                std::flush(tout);
...
void CameraDevice::set_af_area_position_xy(int x, int y)
{
    if (x < 0) x = 0;
    if (x > 639) x = 639;
    if (y < 0) y = 0;
    ...
    // Set AF area position
    SDK::CrDeviceProperty pos;
    pos.SetCode(SDK::CrDevicePropertyCode::CrDeviceProperty_AF_Area_Position);
    pos.SetCurrentValue((CrInt64u)x_y);
    pos.SetValueType(SDK::CrDataType::CrDataType_UInt32);
    SDK::SetDeviceProperty(m_device_handle, &pos);

    // Hold S1 (half-press) to start continuous AF
    // In AF-C mode this keeps tracking; in AF-S it focuses once
    SDK::CrDeviceProperty s1;
    s1.SetCode(SDK::CrDevicePropertyCode::CrDeviceProperty_S1);
    s1.SetCurrentValue(SDK::CrLockIndicator::CrLockIndicator_Locked);
    s1.SetValueType(SDK::CrDataType::CrDataType_UInt16);
    SDK::SetDeviceProperty(m_device_handle, &s1);
    // Keep S1 held - will be released by "afrelease" command or next "af" call
